...
def value_of_ace(card_one, card_two):
    card1 = value_of_card(card_one)
    card2 = value_of_card(card_two)
    if card1 + card2 <= 10 and card_one not in "A" and card_two not in "A" :
        return 11
    else:
        return 1
...
def is_blackjack(card_one, card_two):
    return (card_one in "KQAJ" or card_two in "KQAJ") and int(value_of_card(card_one)) + int(value_of_card(card_two)) == 11  
