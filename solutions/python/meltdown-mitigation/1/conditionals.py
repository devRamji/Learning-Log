...
def reactor_efficiency(voltage, current, theoretical_max_power):
    efficiency = float((voltage*current) / theoretical_max_power)
    if efficiency >= 0.8 :
        return 'green'
    elif  0.6<= efficiency < 0.8:
        return 'orange'
    elif 0.3<= efficiency <0.6:
        return 'red'
    else:
        return 'black'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    if  temperature*neutrons_produced_per_second < 0.9*threshold:
        ...
        return 'LOW'
    elif threshold * 0.9 <= temperature*neutrons_produced_per_second <= 1.1*threshold:
        return 'NORMAL'
    else :
        return 'DANGER'
